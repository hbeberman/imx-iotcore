...
* fTPM secret identity seed derived from secret unique identity retrieved from Trusted/Secure CAAM after High Assurance Boot.

## In-factory First Boot Flow
* U-Boot SPL (SRKH and MAC address Provisioning)
  1) U-Boot SPL boots into *spl_board_manufacture( )* which does the following:
  ...
  3) Requests a MAC address from the manufacturing host and fuses it onto the SoC.
  4) SPL resets the platform.
  5) After the reset *spl_board_manufacture( )* senses that the system is in Trusted state so boot continues.
...
* OP-TEE (RPMB Provisioning)
  1) OP-TEE detects that the RPMB has not been provisioned with an RPMB key and prepares to write one.
  ...
  3) OP-TEE sends the RPMB key to the RPMB through UEFI in plain-text, then verifies that it succeeded.
  4) The *tee_otp_set_rpmb_key_write_lock( )* function blows one of the fuse bits in one of the SoC general purpose fuses so that *tee_otp_check_rpmb_key_write_lock( )* will fail if it's ever called again.
* Late UEFI (EK Certificate Capture, SMBIOS Provisioning)
  1) Once the fTPM is finished initializing UEFI eventually loads the provisioning driver which does the following:
  2) Checks UEFI Variables to see if "DeviceProvisioned" is already set, but it is not so the driver continues.
  3) Sends a message to the manufacturing host over serial to see if it responds correctly.
  4) Retrieves the Endorsement Key Certificate from the fTPM and sends it to the manufacturing host.
  5) Requests the cross-signed version of the Endorsement Key Certificate from the manufacturing host and saves it into a UEFI variable.
  6) Requests per-device SMBIOS customizations from the manufacturing host and saves each one in its own UEFI variable.
  7) Sets "DeviceProvisioned" so that all subsequent boots will return from this driver immediately.
...
1) Copy the code within `#ifdef CONFIG_SPL_BOARD_MANUFACTURE` from board\solidrun\mx6cuboxi\mx6cuboxi.c into your own board specific code. This code includes struct srkfdt and *spl_board_manufacture( )*.

2) Set CONFIG_SPL_BOARD_MANUFACTURE=y in your board defconfig

3) Build the firmware, deploy it to the device, start provision.py from the "imx-iotcore" section below, and attempt to boot. This firmware will not blow any of the fuses, it will just print out what it would blow. This can be used to verify that the SRKH and MAC addresses are correct before permanently commiting them to the platform.

4) Once you've verified that the values are correct you can set CONFIG_MANUFACTURE_FUSES=y in your board defconfig so that spl_board_manufacture will blow the SRKH and MAC fuses.
...
* MFG:reqmac
  * The device is ready to recieve a unique MAC address from the provisioning host.
  ...
  * The host must ensure that each device recieves a unique MAC address from the OEM's assigned MAC addresses.
* MFG:hostcheck
...
* MFG:ekcert
  ...
  * The device sends the EK Certificate buffer recieved from the fTPM.
...
* MFG:devicecert
  * The device is ready to recieve a cross-signed version of the EK Certificate from the provisioning host.
...
* MFG:smbiosserialreq
  * The device is ready to recieve a unique serial number from the provisioning host.
...
* MFG:devicecertfail
  * The device failed to recieve the cross-signed certificate from the host.
* MFG:smbiosfail
  * The device failed to recieve customized SMBIOS values from the host.
* MFG:provisionedfail
...
The flow additionally sets the SEC_CONFIG fuse to 1, putting the paltform into Closed state on boot, meaning the Boot ROM will only start execution of loaded software if its signature matches the keys allowed in the SRKH.
...
When the fTPM and AuthVars TAs are running in OP-TEE they make requests to securely store non-volatile data in the RPMB paritition of an onboard eMMC. In order to handle these writes and reads OP-TEE submits them to the OP-TEE supplicant who runs the storage driver on it's behalf. In the case of this BSP the OP-TEE suplicant RPMB transactions is UEFI, then once Windows is running it's the imxusdhc driver.
...
The UEFI provisioning DXE driver defined in Provisioning.c opens a handle to the fTPM driver and requests the Endorsement Key Certificate. It signals the provisioning host that it's about to send the EK Cert by sending the string "MFG:ekcertstart\n" over serial it then sends the EK Cert over serial along with a length and checksum. The provisioning host then saves the certificate.

Next the device sends "MFG:devicecert\n" to the provisioning host to retrieve the cross-signed version of the certificate. It then recieves a length, the buffer, and a checksum from the host device, and if everything succeded it writes the certificate into the "ManufacturerDeviceCert" UEFI variable.
